...
void sendFic(char* fic,int size,socklen_t alen,int s){
    ...
    int seq=0,i=0,newACK=0,ACKvalide=0,posInFic=0,count=0,tinfs,w=0,ACKproblem=-1,lastseq=-1;
    int windows = 2,bufferFide=1,attente=0,lastSendSeq=-1,maxWindows=64;
    ...
    while(1){
        ...
        //-----------------------------------------------------
            if(FD_ISSET(s, &readset)){
                ...
                if(newACK==indicelastSeq){for(int h = 0;h<10;h++){sendFin(s,alen);}}
