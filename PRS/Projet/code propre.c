...
void sendFic(char* fic,int size,socklen_t alen,int s){
    ...
    int windows = 100,bufferFide=1,attente=0,maxWindows=64,maxSeqSend=0,bug=0;
        ...
        //-----------------------------------------------------
            if(FD_ISSET(s, &readset)){
                ...
                if(newACK==indicelastSeq){for(h = 0;h<10;h++){sendFin(s,alen);}}
                    ...
                    ACKproblem=newACK;
                    if(DEBUG){printf("ACKprblem -> %d",ACKproblem);}
                }else 
                if(DEBUG){printf("Nouvelle ACK inutile\n");}
                count=0;
                if(ACKvalide==maxSeqSend) bufferFide=1;
            }
            else{ 
                count++;
